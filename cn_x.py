...
# 定义日志和模型输出的路径。
MODEL_SAVE_PATH = r"D:\Desktop\Deep_Learning_with_TensorFlow\models\cn_x"
MODEL_NAME = "model.ckpt"
DATA_PATH = r"D:\Desktop\Deep_Learning_with_TensorFlow\output.tfrecords"
...
def main():
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        learning_rate = tf.train.exponential_decay(
            LEARNING_RATE_BASE, global_step, 80000 / BATCH_SIZE, LEARNING_RATE_DECAY)
        opt = tf.train.GradientDescentOptimizer(learning_rate)
        tower_grads = []
        reuse_variables = False
        # 将神经网络的优化过程跑在不同的GPU上。
        for j in range(N_GPU*1):
            i= j % N_GPU
            # 将优化过程指定在一个GPU上。
