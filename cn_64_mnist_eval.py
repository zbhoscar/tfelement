...
import mnist_inference_64 as mnist_inference
import cn_64_mnist_train as mnist_train
import numpy as np
...
def evaluate(mnist):
    with tf.Graph().as_default() as g:
        sample_num = len(mnist.validation.images)
        x =tf.placeholder(tf.float32,[sample_num,mnist_inference.IMAGE_SIZE,
            mnist_inference.IMAGE_SIZE,mnist_inference.NUM_CHANNELS],name='x-input')
        y_=tf.placeholder(tf.float32,[sample_num,mnist_inference.OUTPUT_NODE],name='y-input')
        reshaped_xs=np.reshape(mnist.validation.images,(sample_num,
        ...
        while True:
            with tf.Session(config=config) as sess:
                ...
                if ckpt and ckpt.model_checkpoint_path:
                    ...
                    print('After %s training steps,vld_acc= %g' % (global_step,accuracy_score))
                else:
...
#           _, loss_value,validate_acc,step=sess.run([train_op,loss,accuracy,global_step],feed_dict={x:xs,y_:ys})
